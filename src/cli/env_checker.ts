...
export class EnvChecker {
    ...
    private installBjc(): boolean {
        ...
        // Check if bjc is already installed (bjc.js exists in ets_ui directory)
        let bjcCheckPath = path.join(sdk, 'default', 'openharmony', 'ets', 'build-tools', 'ets-loader', 'lib', 'fast_build', 'ets_ui', 'bjc.js');
        if (fs.existsSync(bjcCheckPath)) {
            logger.info('bjc already installed, skip installation.');
            return true;
        }
        // install bjc to sdk
        if (!install(sdk)) {
